...
* Install the [infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic) on a Linux or Windows OS host, or on a host capable of remotely accessing where SQL Server is installed.

* Requires SQL Server user or domain user with [user privileges](#users-privileges) for `CONNECT`, `VIEW SERVER STATE` and `VIEW ANY DEFINITION` access permissions.
...
4. Edit the `mssql-config.yml` configuration file with your favorite editor. Check out some [configuration file examples.](#examples).

### Linux installation [#linux]

1. Install [the infrastructure agent](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install), and replace the `INTEGRATION_FILE_NAME` variable with `nri-mssql`.
...
For the Microsoft SQL Server to be monitored, you need a user with `CONNECT`, `VIEW SERVER STATE`, and `VIEW ANY DEFINITION` permissions.
...
<CollapserGroup>
  ...
  >
   ...
   1. Create a new login and grant `CONNECT` and `VIEW` permissions:
    ...
    2. Use the following statements to grant DB `public` role privileges to the user:
  ...
  >
    ...
    2. Use the following statements to grant `public` role privileges to the user:
...
To find your integration data, go to <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**</DNT> and select one of the Microsoft SQL Server integration links.
